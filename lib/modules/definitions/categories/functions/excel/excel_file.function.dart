...
Future<void> generateCategoriesExcelFile({
  ...
  try {
    ...
    // Get categories list
    final categoriesList = await CategoriesController.getMany(
    ...
    // cast data to category model list
    final categories = List<Category>.from(categoriesList.data);
    ...
    for (int i = 0; i < categories.length; i++) {
      sheet
          ...
          .value = TextCellValue(categories[i].name);

      sheet
          .cell(
            CellIndex.indexByString("B${i + 2}"),
          )
          .value = TextCellValue(format.format(categories[i].createdAt));

      sheet
          .cell(
            CellIndex.indexByString("C${i + 2}"),
          )
          .value = TextCellValue(format.format(categories[i].updatedAt));
    }
    ...
    String filePath = '${directory.path}/categories.xlsx';
