...
Future<void> generateCustomersExcelFile({
  ...
  try {
    ...
    for (int i = 0; i < customers.length; i++) {
      sheet
          ...
          .value = TextCellValue(customers[i].name);
      ...
      sheet
          .cell(
            CellIndex.indexByString("L${i + 2}"),
          )
          .value = TextCellValue(format.format(customers[i].createdAt));

      sheet
          .cell(
            CellIndex.indexByString("M${i + 2}"),
          )
          .value = TextCellValue(format.format(customers[i].updatedAt));
    }
