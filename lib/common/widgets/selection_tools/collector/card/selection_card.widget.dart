...
import 'package:rst/common/widgets/selection_tools/collector/dialog/selection_dialog.widget.dart';
...
class CollectorSelectionToolCard extends StatefulHookConsumerWidget {
  ...
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _CollectorSelectionToolCardState();
}

class _CollectorSelectionToolCardState
    extends ConsumerState<CollectorSelectionToolCard> {
  ...
  void initState() {
    ...
    if (widget.collector != null) {
      Future.delayed(
        ...
        () {
          ref
              .read(collectorSelectionToolProvider(widget.toolName).notifier)
              .state = widget.collector;
        },
      );
    }
  }
    ...
    return Container(
      ...
      child: InkWell(
        onTap: widget.enabled == null || widget.enabled == true
            ? () async {
                // invalidate collector selection list parameters
                ref.invalidate(collectorsSelectionListParametersProvider);

                FunctionsController.showAlertDialog(
                  context: context,
                  alertDialog:
                      CollectorSelectionDialog(toolName: widget.toolName),
                );
              }
            : null,
          ...
          child: Row(
            children: [
              InkWell(
                onTap: () {
                  ref.invalidate(
                      collectorSelectionToolProvider(widget.toolName));
                },
                child: Icon(
